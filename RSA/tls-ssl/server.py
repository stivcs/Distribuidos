...
print("Servidor seguro iniciado. Esperando conexiones...")
...
def handle_client(conn, addr):
    try:
        ...
        while True:
            ...
            print(f"{sender_username}: {message}")  # Mostrar nombre y mensaje
            ...
            with clients_lock:
                for other_conn in clients:
                    if other_conn != conn:
                        other_conn.send(sender_username.encode())  # Enviar nombre del remitente
                        other_conn.send(encrypted_for_clients)     # Enviar mensaje cifrado
...
# Iniciar el servidor (siempre como servidor)
try:
